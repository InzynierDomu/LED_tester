...
: the_view_mock(new Color_tester_view_mock())
...
{
  EXPECT_CALL(*the_view_mock, clear_cursor());
  EXPECT_CALL(*the_view_mock, update_cursor());
...
{
  EXPECT_CALL(*the_view_mock, clear_cursor()).Times(3);
  EXPECT_CALL(*the_view_mock, update_cursor()).Times(3);
...
{
  EXPECT_CALL(*the_view_mock, update_color_saturation());
